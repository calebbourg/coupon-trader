...
RSpec.describe 'Coupons create' do
  ...
  let(:invalid_params) { attributes_for(:coupon, user_id: user.id,  value: nil) }
    ...
    it_behaves_like 'secured route', '/api/v1/coupons', :post, FactoryBot.attributes_for(:coupon)
    it_behaves_like 'invalid params' do
      let(:route) { '/api/v1/coupons' }
      let(:verb) { :post }
      ...
      it 'creates a new coupon' do
        expect do
          post '/api/v1/coupons', headers: authenticated_header(user), params: { coupon: valid_params }
