...
RSpec.describe 'User update balance' do
  let(:user) { create(:user) }
    ...
    it_behaves_like 'secured route', "/api/v1/users/4", :put, { balance_change: 20 } 

    it 'allows user to add to balance' do
      expect(user.balance).to eq(0.0)
      put "/api/v1/users/#{user.id}", headers: authenticated_header(user), params: { balance_change: 20 }
    ...
    it 'allows user to subtract from balance' do
      ...
      put "/api/v1/users/#{user.id}", headers: authenticated_header(user), params: { balance_change: -15 }
      expect(response).to be_success
